...
export default function Home() {
  ...
  const [passeioSelecionado, setPasseioSelecionado] = useState({ nome: '', id: '' });
  ...
  const abrirReserva = (nome: string, id: string) => {
    setPasseioSelecionado({ nome, id });
    setModalOpen(true);
  };
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900">
              ...
              <div key={tour.id} className="group bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => abrirReserva(tour.title, tour.id)}
                      className="flex-1 bg-[#0a4d54] hover:bg-[#073538] text-white font-bold py-3 rounded-xl text-center transition-all group-hover:scale-[1.02]"
                    >
                      Reservar Online
                    </button>
                    <Link 
                      ...
                      target="_blank"
                      className="w-12 h-12 flex items-center justify-center bg-[#25D366] hover:bg-[#1da851] text-white rounded-xl transition-all"
                      title="WhatsApp"
                    >
      ...
      <ReservaModal
        ...
        passeioId={passeioSelecionado.id}
      />
    </main>
  );
}
